...
## Required parameters
flags.DEFINE_string(
    "bert_config_file", "HanBert-54kN/bert_config.json",
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string("vocab_file", "HanBert-54kN/vocab_v3_54000.txt",
...
def read_squad_examples(is_training, data=None, input_file=None):
  ...
  for entry in input_data:
    for paragraph in entry["paragraphs"]:
      ...
      paragraph_text = normalize_chars(paragraph_text)
      doc_tokens = []
      char_to_word_offset = []
      ...
      for qa in paragraph["qas"]:
        ...
        if is_training:
          ...
          if not is_impossible:
            ...
            orig_answer_text = normalize_chars(orig_answer_text)
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      train_op = optimization.create_optimizer(
          total_loss, learning_rate, num_train_steps, num_warmup_steps, use_tpu, local_distributed=False)
...
#          if not feature.token_is_max_context.get(start_index, False): continue
    ...
    if not FLAGS.version_2_with_negative:
      ...
      if FLAGS.korquad_refine_answer_by_pos:
          tagged_words = pos_analyzer.pos(nbest_json[0]["text"])
          if tagged_words[-1][1].startswith("J") and (ord("가") < ord(tagged_words[-1][0][0]) < ord("힣")):
              print("original text :", nbest_json[0]["text"])
              nbest_json[0]["text"] = nbest_json[0]["text"][:-len(tagged_words[-1][0])]
              print("refined text :", nbest_json[0]["text"])
          # nbest_json[0]["text"] = " ".join(tagged_words)
...
class FeatureWriter(object):
  ...
  def __init__(self, filename, is_training, record_file_exists=False):
    self.filename = filename
    self.is_training = is_training
    self.num_features = 0
    self.record_file_exists = record_file_exists
    if not self.record_file_exists:
